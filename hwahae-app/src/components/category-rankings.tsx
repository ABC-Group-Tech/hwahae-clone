...
function GoldMedal() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="37" fill="none" viewBox="0 0 30 36" className="mb-2">
      ...
      <path
        fill="#fff"
        d="M14.945 16.75H16.5v9h-1.893v-7.104l-2.107.911v-1.746l2.445-1.05z"
      />
    </svg>
  )
}
...
export default function CategoryRankings() {
  ...
  return (
    <section className="mt-[40px]">
        ...
        <div className="px-5">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold leading-normal">
              ABC Pharm 고객들이 직접 <span className="text-[#f39800]">선택한 랭킹</span>
            </h2>
      ...
      {/* Category Filter */}
      <div className="flex items-center px-5 h-[52px] mt-2 [&>label]:shrink-0">
        <div className="inline-flex shrink-0 relative z-[1]" ref={dropdownRef}>
          <label className="inline-flex items-center text-sm text-gray-900" htmlFor="category-toggle">
            <input
              className="hidden"
              id="category-toggle"
              type="checkbox"
          ...
          <div className="w-12 h-8 bg-gradient-to-l from-white/0 from-0% via-white/50 via-25% to-white/100 to-100%"></div>

          {/* Dropdown Menu */}
          {isDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 w-48 bg-white rounded-lg border border-gray-300 shadow-lg z-10 py-1 max-h-[300px] overflow-y-auto">
              {categories.map((category) => (
                ...
                >
                  <span className={selectedCategory === category ? "text-[#f39800] font-medium" : "text-gray-900"}>
                    {category}
                  </span>
